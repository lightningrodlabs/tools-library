...
pub fn validate_create_link_developer_collective_to_contributor_permissions(
    ...
    let developer_collective_action_hash = base_address
        .into_action_hash()
        .ok_or(
            wasm_error!(
    ...
    let developer_collective_record = must_get_valid_record(
        developer_collective_action_hash.clone(),
    )?;
    ...
    let contributor_permission_action_hash = target_address
        .into_action_hash()
        .ok_or(
            wasm_error!(
                WasmErrorInner::Guest("No action hash associated with link".to_string())
    ...
    let contributor_permission_record = must_get_valid_record(
        contributor_permission_action_hash,
    )?;
    ...
    let agent_in_tag = AgentPubKey::from_raw_39(tag.0.clone())
        .map_err(|_| {
            wasm_error!(
                WasmErrorInner::Guest("Link tag does not contain a valid agent public key"
                .into())
            )
        })?;
...
pub fn validate_create_link_contributor_to_contributor_permissions(
    ...
    let contributor_agent_key = base_address
        .into_agent_pub_key()
        .ok_or(
            wasm_error!(WasmErrorInner::Guest("No agent hash as link base.".to_string())),
        )?;
    let developer_collective_in_tag = ActionHash::from_raw_39(tag.0.clone())
        .map_err(|_| {
            wasm_error!(
                WasmErrorInner::Guest("Link tag does not contain a valid action hash"
                .into())
            )
        })?;
    if developer_collective_in_tag != contributor_permission.for_collective {
        return Ok(
            ValidateCallbackResult::Invalid(
                "Link tag does not contain the action hash of the DeveloperCollective for which the ContributorPermission is for."
                    .into(),
            ),
        );
    }
