...
pub fn validate_create_link_developer_collective_updates(
    ...
    let _developer_collective: crate::DeveloperCollective = collective_record
        ...
        .ok_or(
            wasm_error!(
                WasmErrorInner::Guest("Link base must be DeveloperCollective entry"
                .to_string())
            ),
        )?;
    let collective_update_action_hash = target_address
        .into_action_hash()
        .ok_or(
            wasm_error!(
                WasmErrorInner::Guest("No action hash associated with link".to_string())
    ...
    let _developer_collective: crate::DeveloperCollective = collective_update_record
        ...
        .ok_or(
            wasm_error!(
                WasmErrorInner::Guest("Link target must be DeveloperCollective entry"
                .to_string())
            ),
        )?;
    Ok(ValidateCallbackResult::Valid)
