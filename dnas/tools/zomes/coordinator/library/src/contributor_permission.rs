    ...
    match developer_collective_record {
        Some(r) => {
            ...
            let links_to_my_permissions = permission_links
                .into_iter()
                .filter_map(|link| {
                    let maybe_permitted_agent = AgentPubKey::from_raw_39(
                            link.tag.0.clone(),
                        )
                        .ok();
            ...
            match links_to_my_permissions.len() {
                ...
                _ => {
                    ...
                    for link in links_to_my_permissions {
                        let maybe_permission_action_hash = ActionHash::try_from(
                                link.target,
                            )
                            .ok();
                        if let Some(permission_action_hash) = maybe_permission_action_hash {
                            let maybe_permission_record = get(
                                    ...
                                    Ok(maybe_permission) => {
                                        if let Some(permission) = maybe_permission {
                                            match permission.expiry {
                                                None => {
                                                    return Ok(Some(permission_record.action_address().clone()));
                                                }
                                                Some(expiry) => {
                                                    expiring_permissions
                                                        .push((expiry, permission_record.action_address().clone()));
                                                }
                                            }
                                        }
                                    }
                    ...
                    match max_expiry_permission {
                        Some((expiry, action_hash)) => {
                            let now = sys_time()?;
                            if now > expiry { Ok(None) } else { Ok(Some(action_hash)) }
                        }
                        None => Ok(None),
                    }
                }
